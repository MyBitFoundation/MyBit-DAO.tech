...
contract Kit is KitBase {
    ...
    function setupDAO(Kernel _dao, ACL _acl) internal {
      ...
      MyTokens myTokens = MyTokens(_dao.newAppInstance(myTokensAppId, latestVersionAppBase(myTokensAppId)));
      ...
      myTokens.initialize(token, address(erc20), address(tokensale), address(0), lockAmounts, lockIntervals, tokenIntervals);
      //voting.initialize(token, 50 * PCT, 20 * PCT, 1 days);
      //myID.initialize(whitelist);


      //_acl.createPermission(this, myTokens, myTokens.LOCK_ROLE(), this);
      //myTokens.mint(root, 1); // Give one token to root

      setupPermissions(_dao, _acl, /*voting,*/ myTokens, vault, /*finance,*/ root);
    }

    function setupPermissions(Kernel _dao, ACL _acl, /*Voting _voting,*/ MyTokens _myTokens, Vault _vault, /*Finance _finance,*/ address _root) internal{
      //_acl.createPermission(ANY_ENTITY, _voting, _voting.CREATE_VOTES_ROLE(), _root);
      _acl.createPermission(_myTokens, _myTokens, _myTokens.BURN_ROLE(), _myTokens);
      _acl.createPermission(_myTokens, _myTokens, _myTokens.MANAGER_ROLE(), _myTokens);
      _acl.createPermission(_myTokens, _vault, _vault.TRANSFER_ROLE(), _myTokens);

      //_acl.createPermission(_finance, _vault, _vault.TRANSFER_ROLE(), _myTokens);
      //_acl.createPermission(_myTokens, _finance, _finance.CREATE_PAYMENTS_ROLE(), _myTokens);
      //_acl.createPermission(_myTokens, _finance, _finance.EXECUTE_PAYMENTS_ROLE(), _myTokens);
      //_acl.createPermission(_myTokens, _finance, _finance.MANAGE_PAYMENTS_ROLE(), _myTokens);
